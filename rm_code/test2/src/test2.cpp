...
private:
    ...
    void pnpsolver(){
        cv::Mat rvec, tvec;
        rm_auto_aim::Armor armor(lights[0], lights[1]);
        armor.type = rm_auto_aim::ArmorType::SMALL;
        solvePnP(armor, rvec, tvec);
        //
        //暂时不知道是否要进行判断
        distance =calculateDistanceToCenter(armor.center);
    }

    bool solvePnP(const rm_auto_aim::Armor & armor, cv::Mat & rvec, cv::Mat & tvec){
        ...
        // Solve pnp
        auto object_points = armor.type == rm_auto_aim::ArmorType::SMALL ? small_armor_points_ : large_armor_points_;
        return cv::solvePnP(
